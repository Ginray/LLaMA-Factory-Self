...
def get_template_and_fix_tokenizer(
    ...
    if template.replace_eos: #如果模板指示需要替换 EOS（End of String）标记，且模板还指定了停用词，则取出第一个停用词在分词器中替换 EOS 标记。，并将剩余的停用词保存起来。
        if not stop_words:
    ...
    if tokenizer.eos_token_id is None: # 如果分词器中没有定义 EOS 标记，则在分词器中添加一个空EOS 标记"<|endoftext|>"。
        _add_or_replace_eos_token(tokenizer, eos_token="<|endoftext|>")

    if tokenizer.pad_token_id is None: # 如果分词器中没有定义 PAD 标记，则将 PAD 标记设置为与 EOS 标记相同的值，并在日志中记录
        tokenizer.pad_token = tokenizer.eos_token
        logger.info("Add pad token: {}".format(tokenizer.pad_token))
    # 停用词主要的作用就是忽略掉
    if stop_words: # 如果模板指定了停用词，并且有剩余的停用词，则将这些停用词添加到分词器的特殊标记中，并在日志中记录。如果成功添加了新的特殊标记，则会发出警告提醒用户确认是否需要调整词汇表大小。
    ...
    try: # HF的Jinja 模板，可将对话历史记录格式化为正确的字符串
